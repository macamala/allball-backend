    ...
    try:
        ...
        if writes_enabled():
            ...
            try:
                ...
                if search_assets:
                    logger.info("PRELOCK_FOTMOB_SEARCH_ASSETS %s", search_assets)
            except Exception:
                logger.exception("Pre-lock FotMob search artwork repair failed")
                db.rollback()
...
def main(once: bool = True, interval_seconds: Optional[int] = None) -> None:
    ...
    # The production API owns schema/bootstrap DDL. Worker startup must not
    # reflect the whole database or run DDL before score refresh; that can block
    # behind unrelated database activity and delay live results.
    logger.info("Results worker startup DDL delegated to API")
